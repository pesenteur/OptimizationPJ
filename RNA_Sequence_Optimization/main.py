...
from sequence import generate_candidates, compute_score
...
def main():
    ...
    if args.method == 'A':
        ...
        optimized_sequence = optimize_method_a(initial_sequence, args.lambda, args.iterations)
    
    elif args.method == 'B':
        print("开始方法B: 连续优化 + 牛顿法")
        optimized_sequence = optimize_method_b(initial_sequence, args.lambda, args.iterations)
    
    elif args.method == 'C':
        print("开始方法C: 混合启发式优化 + 连续优化 + 局部搜索")
        optimized_sequence = optimize_method_c(initial_sequence, args.lambda, args.iterations)

    # 4. 计算优化后的序列的目标函数得分
    final_score = compute_score(optimized_sequence)
